...
{
 $this->sender     =  Set::set()->sms_name;                    
 $this->timeSend   = 0; 
 $this->dateSend   = 0;                             
 $this->deleteKey  = 152485;                                                 
 $this->resultType = 1;                  
 $this->viewResult = 1;        
 $this->MsgID       = rand(0000,9999);             
 $this->userAccount = Set::set()->sms_user;                  
 $this->passAccount = Set::set()->sms_pass;  
}
...
public function sendSMS($numbers,$msg)
{
    ...
    $applicationType = "24";  
    $msgs         = $this->convertToUnicode($msg);// ;
    //return dd($msgs);
    $sender       = urlencode($this->sender);
    $domainName   = $_SERVER['SERVER_NAME'];
    $stringToPost = "mobile=".$this->userAccount."&password=".$this->passAccount."&numbers=".$numbers."&sender=".$sender."&msg=".$msgs."&timeSend=".$this->timeSend."&dateSend=".$this->dateSend."&applicationType=".$applicationType."&domainName=".$url."&msgId=".$this->MsgID."&deleteKey=".$this->deleteKey;
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_HEADER, 0);
   ...
   public function sendmsg(Request $r)
   {
    if($r->has('level'))
    {
     ...
     $rules = ['number'=>'required','msg'=>'required'];
    }
    $validator = Validator::make($r->all(),$rules);
    $validator->SetAttributeNames(['number'=>trans('admin.number'),'msg'=>trans('admin.msg')]);
    if($validator->fails())
    {
       return redirect()->back()->withInput()->withErrors($validator);
        ...
        if($r->has('level'))
        {
            $users = User::where('level','=',$r->input('level'))->get();
            foreach($users as $user)
            {
            $sms->sendSMS('966'.$user->mobile,$r->input('msg'));
            }
        }else{
            $sms->sendSMS($r->input('number'),$r->input('msg'));
        }
        session()->flash('success',trans('admin.msgsent'));
