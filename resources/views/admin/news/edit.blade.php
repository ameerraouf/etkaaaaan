...
{!! Form::open(['url'=>app('aurl').'/news/'.$edit->id,'method'=>'put','files'=>true]) !!}
<input type="hidden" name="id_other_files" class="id_other_files" />
<div class="form-group">
<label>{{ trans('admin.title') }}</label>
<input name="title" type="text" placeholder="{{ trans('admin.title') }}" class="form-control" value="{{ $edit->title }}">
</div>
...
@if(!empty($edit->photo))
 <img src="{{ url('upload/'.$edit->photo) }}" style="width:100px;height:100px;" />
@endif
</div>
 
<div class="form-group">
<label>{{ trans('admin.department') }}</label>
{!! Form::select('department',App\Department::pluck('title','id'),$edit->department,['class'=>'form-control']) !!}
</div>

<div class="form-group">
<label>{{ trans('admin.content') }}</label>
{!! Form::textarea('content',$edit->content,['class'=>'form-control ckeditor']) !!}
</div>


<?php $youtube = explode('||',$edit->youtube); ?>
<div class="form-group">
<label>{{ trans('admin.youtube') }}  1 </label>
<input name="youtube[]" type="text" placeholder="{{ trans('admin.youtube') }}" class="form-control" value="{{ @$youtube[0] }}" placeholder="{{ trans('admin.youtube') }}">
</div>
 
<div class="form-group">
<label>{{ trans('admin.youtube') }}  2 </label>
<input name="youtube[]" type="text" placeholder="{{ trans('admin.youtube') }}2" class="form-control" value="{{ @$youtube[1] }}" placeholder="{{ trans('admin.youtube') }}">
</div>
...
<ol class="list_attach" style="margin:0;padding:5px;direction:ltr;">
 @foreach($edit->files()->get() as $file)
  @if($file->ext == 'mp3')
   @include(app('at').'.news.audiofile',['file'=>$file])
  @else
   @include(app('at').'.news.photofile',['file'=>$file])
  @endif
 @endforeach	
</ol>
