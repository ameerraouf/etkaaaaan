...
<link href="{{url('assets')}}/css/pages/login-rtl.css" rel="stylesheet" type="text/css"/>
...
<div class="content">
	<!-- BEGIN LOGIN FORM -->
	<form class="login-form" action="{{ url(app('aurl').'/login') }}" method="post">
		 <input type="hidden" name="_token" value="{{ csrf_token() }}">
		<h3 class="form-title">{{ trans('admin.login_account') }}</h3>
	 
		<div class="alert alert-danger display-hide">
			<button class="close" data-close="alert"></button>
			<span>
				 {{ trans('admin.messageerrorlogin') }}
			</span>
		</div>
		<div class="form-group">
			...
			<div class="input-icon">
				...
				<input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="{{ trans('admin.email') }}" name="email"/>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="input-icon">
				...
				<input class="form-control placeholder-no-fix" type="password" autocomplete="off" placeholder="{{ trans('admin.password') }}" name="password"/>
			</div>
		</div>
		<div class="form-actions">
			<label class="checkbox">
			<input type="checkbox" name="rememberme" value="1"/> {{ trans('admin.rememberme') }} </label>
...
<!-- BEGIN COPYRIGHT -->
<div class="copyright">
 برمجة وتطوير كوكبة التقنية {{ date('Y') }}
</div>
<!-- END COPYRIGHT -->
